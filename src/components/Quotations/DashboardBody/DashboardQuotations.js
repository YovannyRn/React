...
function DashboardQuotations() {
  ...
  const respondedSuppliers = new Set(
    quotations.filter(q => q.status !== "Draft" && q.status !== "Sent").map(q => q.supplierId)
  );
  const totalSuppliers = new Set(quotations.map(q => q.supplierId));
  const responseRate = totalSuppliers.size > 0 ? Math.round((respondedSuppliers.size / totalSuppliers.size) * 100) : 0;
  const avgLeadTime = quotations.length > 0
    ? (
        quotations.filter(q => typeof q.deliveryTime === "number").reduce((acc, q) => acc + q.deliveryTime, 0) /
  ...
  quotations.forEach(q => {
    if (q.creationDate) {
      const days = Math.floor((now - new Date(q.creationDate)) / (1000 * 60 * 60 * 24));
  ...
  const pendingEvaluation = quotations.filter(q => q.status === "Under Review").length;
  const budgetDeviation = "-5%"; // Ficticio
  const bestOfferSavings = "$1,200"; // Ficticio
  const rejectionRate = totalQuotations > 0 ? Math.round((quotations.filter(q => q.status === "Rejected").length / totalQuotations) * 100) : 0;
  const allQuotationValue = quotations.reduce((acc, q) => acc + (q.totalAmount && q.totalAmount.$numberDecimal ? parseFloat(q.totalAmount.$numberDecimal) : 0), 0);


  const agingDays = quotations
  .filter(q => q.creationDate)
  .map(q => Math.floor((now - new Date(q.creationDate)) / (1000 * 60 * 60 * 24)));
  const avgAging = agingDays.length > 0 ? (agingDays.reduce((a, b) => a + b, 0) / agingDays.length).toFixed(1) : 0;
  ...
  {
    icon: <PendingActionsIcon fontSize="large" className="text-warning me-2" />,
  ...
  const currentKpis = kpis.slice(kpiPage * kpisPerPage, (kpiPage + 1) * kpisPerPage);
  ...
  <div className="container mt-2">
    ...
    {showKPIs && (
      <div className="d-flex justify-content-center align-items-center" style={{ minHeight: 100 }}>
          <KeyboardArrowLeftIcon type="button" className="text-center justify-content-center me-4 text-primary" onClick={() => setKpiPage((kpiPage + 1) % totalPages)}
          title="Siguiente KPIs"/>
        <div className="d-flex justify-content-center align-items-center gap-3 flex-nowrap">
          {currentKpis.map((kpi, idx) => (
            <div
              key={idx}
              className="d-flex align-items-center p-3 shadow-sm border-0"
              style={{
                borderRadius: "10px",
            ...
            >
              ...
              <div className="flex-grow-1">
                ...
                <strong className="d-block mt-1" style={{ fontSize: "1.1rem" }}>{kpi.value}</strong>
              </div>
            </div>
          ))}
        </div>
        
          <KeyboardArrowRightIcon type="button" className="text-center justify-content-center ms-4 text-primary" onClick={() => setKpiPage((kpiPage + 1) % totalPages)}
          title="Siguiente KPIs" />
